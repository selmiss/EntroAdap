...
class MultiModalInferenceDataset(Dataset):
    """
    Dataset for inference with graph structures and text prompts.
    
    Similar to MultiModalSFTDataset but:
    - Removes the last assistant message (to be generated by model)
    - Preserves the assistant message as reference_text for evaluation
    - No masking applied to graphs (full structure visible)
    
    Supports same input formats as MultiModalSFTDataset:
    1. JSONL + separate parquet
    2. Combined parquet
    
    Args:
        dataset_path: Path to JSONL or combined parquet file
        graph_parquet_path: Optional path to separate parquet with graphs
        structure_dir: Directory containing PDB structures
        cache_dir: Cache directory for HF datasets
        use_combined_parquet: If True, load from combined parquet format
        max_atoms: Optional maximum atoms per structure
        max_edges: Optional maximum edges per structure
        skip_on_error: If True, skip samples that fail to load
    """
    ...
    def _load_graph_from_parquet_row(self, row) -> Optional[Dict[str, Any]]:
        """Load graph structure from a combined parquet row (reuses logic from MultiModalSFTDataset)."""
        # Early size check before tensor conversion
        if self.max_atoms is not None or self.max_edges is not None:
            # Check atom count
            if hasattr(row['node_feat'], '__len__'):
        ...
        # Convert nested arrays properly (NO MASKING for inference)
        node_feat_arr = np.array(row['node_feat'])
        if node_feat_arr.dtype == object:
            node_feat_arr = np.vstack([np.array(x) for x in node_feat_arr])
    ...
    def _load_graph_from_source(self, structure_info: Dict[str, Any]) -> Optional[Dict[str, Any]]:
        """Load graph structure from various sources (reuses logic from MultiModalSFTDataset)."""
        source_type = structure_info.get('type')
        value = structure_info.get('value')
        modality = structure_info.get('modality', 'protein')
    ...
    def __getitem__(self, idx: int) -> Dict[str, Any]:
        """
        Get a single example with prompt and optional graph structure.
        
        Returns:
            Dictionary with:
                - messages: List of chat messages (WITHOUT last assistant message)
                - reference_text: Expected response (last assistant message)
        ...
        try:
            if self.use_combined_parquet:
                ...
                # Separate prompt from reference
                prompt_messages = []
                reference_text = ""
                ...
                result = {
                    'messages': prompt_messages,  # Only prompt, no assistant response
                    'reference_text': reference_text,
                    'structure_token': '<STRUCTURE>',
                    'sample_id': f'sample_{idx}',
                }
                
                # Load graph structure (may return None if filtered)
                graph_data = self._load_graph_from_parquet_row(row)
                if graph_data is not None:
                    result['graph_data'] = graph_data
                elif self.skip_on_error and (self.max_atoms is not None or self.max_edges is not None):
